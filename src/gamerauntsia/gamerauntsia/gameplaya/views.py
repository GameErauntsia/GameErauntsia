...
def index(
    request, kategoria=None, username=None, maila=None, jokoa=None, plataforma=None
):
    if kategoria:
        ...
        gameplayak = GamePlaya.objects.filter(
            status="1", publikoa_da=True, zailtasuna=maila, pub_date__lt=timezone.now()
        ).order_by("-pub_date")
    elif jokoa:
        jokoa = get_object_or_404(Jokoa, slug=jokoa)
        gameplayak = GamePlaya.objects.filter(
            status="1", publikoa_da=True, jokoa=jokoa, pub_date__lt=timezone.now()
        ).order_by("-pub_date")
    elif plataforma:
    ...
    plataformak = (
        Plataforma.objects.exclude(gameplay__isnull=True)
        .order_by("izena")
        .select_related("icon")
    )
...
def gameplaya(request, slug, related=None):
    ...
    if gp.status == "0" and not request.user.is_staff:
        raise Http404
    related_gps = (
        GamePlaya.objects.filter(
            status="1", publikoa_da=True, jokoa=gp.jokoa, pub_date__lt=gp.pub_date
        )
        .exclude(id=gp.id)
        .order_by("-pub_date")[:5]
    )
