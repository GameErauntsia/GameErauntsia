...
def index(request):
    ...
    items = (
        ...
        .order_by("jokoa__izena")
        .select_related("jokoa__logoa", "itzulpena")
    )
    links = Euskalinkak.objects.filter(publikoa_da=True).select_related("irudia")
    return render(request, "jokoen_itzulpenak/index.html", locals())


def search_retro(request):
    q = request.GET.get("term", "")
    items = Itzulpena.objects.filter(
        Q(publikoa_da=True),
        Q(izena__icontains=q)
        | Q(jokoa__izena__icontains=q)
        | Q(plataforma__izena__icontains=q)
        | Q(status=q),
    )[:20]
    return render(request, "jokoen_itzulpenak/index.html", locals())
