...
class ItzulpenProiektua(JokoItzulpena):
    ...
    parte_hartzaileak = models.ManyToManyField(
        GamerUser, through="ItzulpenProiektuParteHartzailea"
    )
    ...
    def __str__(self):
        return "Itzulpen proiektua - %s" % (self.jokoa.izena)
...
class ItzulpenFitxategia(models.Model):
    ...
    deskribapen_laburra = models.CharField(
        max_length=300, verbose_name="Deskribapen laburra"
    )
    fitxategia = models.FileField(
        null=True, blank=True, upload_to="itzulpen_fitxategiak"
    )
...
class KanpokoItzulpena(JokoItzulpena):
    ...
    def __str__(self):
        return "Kanpoko itzulpena - %s" % (self.jokoa.izena)
