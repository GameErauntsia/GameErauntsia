...
def index(request):
    items = (
        Txapelketa.objects.filter(publikoa_da=True)
        .order_by("-pub_date")
        .select_related("irudia", "jokoa")
    )
...
def txapelketa(request, slug):
    item = get_object_or_404(
        Txapelketa.objects.select_related("irudia").prefetch_related(
            "jokalariak__photo", "adminak__photo"
        ),
        slug=slug,
    )
    list_sailkapena = item.get_partaideak(
        ["-points", "-win", "lose", "-average"]
    ).prefetch_related("jokalariak__photo")
    single_gamers = item.get_single_gamers()
    partidak = (
        Partida.objects.filter(txapelketa=item)
        .order_by("date")
        .prefetch_related("partaideak__jokalariak")
    )
    video_parts = (
        Partida.objects.filter(Q(txapelketa=item), Q(bideoa__isnull=False))
        .exclude(bideoa__iexact="")
        .order_by("-date")[:3]
    )
    next_parts = (
        Partida.objects.filter(
            Q(txapelketa=item),
            Q(partaideak__isnull=False),
            Q(emaitza__isnull=True) | Q(emaitza__iexact=""),
        )
        .order_by("date", "jardunaldia")
        .distinct()
    )
...
def zuhaitza(request, slug):
    ...
    jardunaldi_kopurua = Partida.objects.filter(txapelketa=item).aggregate(
        Max("jardunaldia")
    )["jardunaldia__max"]
    roundlabels = [
        "{}. jardunaldia".format(zbk + 1) for zbk in range(jardunaldi_kopurua)
    ]

    def get_partida(partida):
        return [
            {"name": gamer.get_izena(), "seed": gamer.id, "id": gamer.id}
            for gamer in partida.partaideak.all()
        ]

    def get_jardunaldi_partidak(jardunaldi_zbk):
        jardunaldia = Partida.objects.filter(
            Q(txapelketa=item), Q(jardunaldia=jardunaldi_zbk + 1)
        ).prefetch_related("partaideak__jokalariak")
        return [get_partida(partida) for partida in jardunaldia]

    graphdata = [
        get_jardunaldi_partidak(jardunaldia)
        for jardunaldia in range(jardunaldi_kopurua)
    ]

    try:
        irabazlea = Partaidea.objects.get(txapelketa=item, irabazlea=True)
        azkena = [
            [{"name": irabazlea.get_izena(), "seed": irabazlea.id, "id": irabazlea.id}]
        ]
    except:
...
def partida(request, slug, partida):
    ...
    other_parts = (
        Partida.objects.filter(txapelketa=item)
        .exclude(id=partida.id, emaitza__isnull=False)
        .order_by("date", "jardunaldia")
        .distinct()
    )
    ...
    if request.method == "POST":
        ...
        if teamform.is_valid():
            ...
            return HttpResponseRedirect(
                reverse("partaidea", kwargs={"part_id": team.id})
            )
    else:
        teamform = TaldeaForm()
