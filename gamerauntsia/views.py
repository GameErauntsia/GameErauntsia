...
def index(request):
    ...
    if Txapelketa.objects.filter(Q(publikoa_da=True),Q(pub_date__lt=datetime.now()),Q(twitch=True),Q(status='2')).exists():
        txs = Txapelketa.objects.filter(Q(publikoa_da=True),Q(pub_date__lt=datetime.now()),Q(twitch=True),Q(status='2'))
        match = None
        last_tx = None
        for tx in txs:
            if not match:
