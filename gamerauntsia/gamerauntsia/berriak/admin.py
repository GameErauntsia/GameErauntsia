...
class BerriakAdmin(admin.ModelAdmin):
    ...
    list_display = (
        'izenburua', 'preview', 'erabiltzailea', 'pub_date', 'mod_date', 'publikoa_da', 'status', 'admin_thumbnail')
    ...
    def get_readonly_fields(self, request, obj=None):
        if request.user.is_superuser or (
                        obj and request.user != obj.erabiltzailea and request.user.belongs_group(settings.NEWS_GROUP)):
    ...
    def has_change_permission(self, request, obj=None):
        ...
        if request.user.is_superuser or obj.erabiltzailea == request.user or request.user.belongs_group(
                settings.NEWS_GROUP):
            return True
        else:
            return False
