...
def index(request,kategoria=None,username=None,maila=None,jokoa=None,plataforma=None):
    if kategoria:
        ...
        gameplayak = GamePlaya.objects.filter(status='1',publikoa_da=True,kategoria=kategoria, pub_date__lt=datetime.now()).order_by('-pub_date')
    elif maila:
        maila = get_object_or_404(Zailtasuna,slug=maila)
        gameplayak = GamePlaya.objects.filter(status='1',publikoa_da=True,zailtasuna=maila, pub_date__lt=datetime.now()).order_by('-pub_date')
    elif jokoa:
        jokoa = get_object_or_404(Jokoa,slug=jokoa)
        gameplayak = GamePlaya.objects.filter(status='1',publikoa_da=True,jokoa=jokoa, pub_date__lt=datetime.now()).order_by('-pub_date')
    elif plataforma:
