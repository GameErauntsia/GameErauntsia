...
def valid_twitch_signature(request):
    try:
        ...
        hmac_message = (
            ...
            + bytes(message_timestamp, "utf-8")
            + request.body
        )
        key = bytes(settings.STREAMING_TWITCH_WEBHOOK_SECRET, "utf-8")
        signature = hmac.new(key, hmac_message, hashlib.sha256)
        ...
        if channel_info:
            ...
            if elapsed_time > timedelta(minutes=10):
                ...
                if channel_info["title"]:
                    msg = "%s %s (%s) stremeatzen ari da! https://twitch.tv/%s" % (
                    ...
                    msg = "%s %s stremeatzen ari da! https://twitch.tv/%s" % (
                        channel_info["broadcaster_name"],
                        channel_info["game_name"],
                        channel_info["broadcaster_name"],
                    )
                tb.send_message(
                    settings.PUBLIC_CHAT_ID, msg, disable_web_page_preview=True
                )
    except:
...
def streaming_ended(event):
    try:
        user_id = event["broadcaster_user_id"]
        user = GamerUser.objects.filter(twitch_channel_id=user_id).first()
        Streaming.objects.filter(user=user, end_date=None).update(
