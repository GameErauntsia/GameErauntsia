...
class Itzulpena(models.Model):
    ...
    status = models.CharField(
        max_length=1, choices=IPS_STATUS, default="0", verbose_name="Egoera"
    )
...
class EuskarazkoJokoa(models.Model):
    ...
    itzulpena = models.ForeignKey(
        Itzulpena, blank=True, null=True, on_delete=models.SET_NULL
    )
    garatzaileak_itzulia = models.BooleanField(
    ...
    def __str__(self):
        return "%s %s" % (self.jokoa.izena, self.jokoa.bertsioa)
    ...
    def is_ge_translation_icon(self):
        return (
            self.is_ge_translation()
            and mark_safe(
                '<i class="glyphicon glyphicon-file"></i> Fitxategia %s'
                % self.itzulpena.get_status_display().lower()
            )
            or ""
        )
