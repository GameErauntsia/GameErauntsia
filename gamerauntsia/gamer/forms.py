...
class ProfilePhotoForm(forms.Form):
    avatarpic = forms.ImageField(
        label=_("Your picture"),
        help_text=_(
            "Please upload a picture of you. Supported formats: jpg, png, gif."
        ),
    )
    ...
    def clean_argazkia(self):
        ...
        try:
            ...
            raise forms.ValidationError(
                _(
                    "The name of the picture (%s) has an unsupported character. Please rename it before uploading."
                )
                % name
            )

        format = name.split(".")[-1]
        if format.lower().strip() == "bmp":
            raise forms.ValidationError(
                _(
                    "The picture is not in one of our supported formats. We don't support BMP files. Please change it"
                )
            )
...
class GamerForm(forms.ModelForm):
    ...
    signature = forms.CharField(
        widget=TinyMCE(
            ...
            mce_attrs=TINYMCE_SMALL_BODY_CONFIG,
        ),
        required=False,
    )
...
class ChannelsForm(forms.ModelForm):
    class Meta:
        model = GamerUser
        widgets = {
            "channel_description": forms.Textarea(),
            "twitch_channel": forms.TextInput(
                attrs={"placeholder": "Erabiltzaile izena, ez URLa"}
            ),
...
class ArticleForm(forms.ModelForm):
    ...
    gaia = forms.ModelMultipleChoiceField(
        label="Gaiak",
        queryset=Gaia.objects.all(),
        widget=forms.SelectMultiple(attrs={"size": "15"}),
        help_text="Aukeratu artikuluarekin zer ikusia duen gai bat edo gehiago",
    )

    argazkia = forms.ImageField(
    ...
    def clean_desk(self):
        ...
        if not desk:
            raise forms.ValidationError(
                "Mezu hutsek ez dute balio. Mesedez, idatzi zerbait!"
            )
...
class GameCatalogForm(forms.ModelForm):
    bertsioa = forms.CharField(
        label="Bertsioa (aukerazkoa)",
        help_text="Joko saga bat bada, zehaztu hemen bertsioa. ",
        required=False,
    )
    logoa = forms.ImageField(
        label="Jokoaren logo ofiziala",
        help_text="Onartutako formatuak: jpg, png, gif.",
        required=False,
    )
    karatula = forms.ImageField(
        label="Jokoaren karatula",
        help_text="Onartutako formatuak: jpg, png, gif.",
        required=False,
    )
    desk = forms.CharField(
        label="Deskribapen laburra (aukerazkoa)", widget=forms.Textarea, required=False
    )
    trailer = forms.CharField(
        label="Youtube trailerra (aukerazkoa)",
...
class GamePlayForm(forms.ModelForm):
    ...
    jokoa = forms.ModelChoiceField(
        label="Jokoa", queryset=Jokoa.objects.all().order_by("izena")
    )

    bideo_plataforma = forms.ModelChoiceField(
        label="Bideo plataforma",
    ...
    lizentzia = forms.BooleanField(
        label="Irakurri eta onartzen ditut gameplayak igotzeko arauak",
        help_text=mark_safe(
            'Informazioa gehiago <a href="/gameplay-arauak">gameplay arauetan</a>.'
        ),
    )
    ...
    def clean_lizentzia(self):
        ...
        if not lizentzia:
            raise forms.ValidationError(
                "Gameplaya igotzeko arauak onartzea beharrezkoa da. Mesedez, irakurri eta onartu arauak."
            )
    ...
    def clean_iraupena_min(self):
        ...
        if minutu == 0:
            raise forms.ValidationError(
                "Bideoaren iraupena zehaztea garrantzitsua da. Mesedez, sartu denbora!"
            )
    ...
    def clean_bideoa(self):
        ...
        if not bideoa:
            ...
            raise forms.ValidationError(
                "Bideoaren kodea bakarrik jarri behar da. Mesedez, ikusi adibidea eta zuzendu kodea!"
            )
        return bideoa
