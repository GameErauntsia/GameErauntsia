...
class GamePlaya(models.Model):
    ...
    plataforma = models.ForeignKey(
        Plataforma, related_name="gameplay", on_delete=models.PROTECT
    )
    zailtasuna = models.ForeignKey(
        Zailtasuna, related_name="gameplay", on_delete=models.PROTECT
    )
    kategoria = models.ManyToManyField(Kategoria, related_name="gameplay")

    erabiltzailea = models.ForeignKey(
        GamerUser, related_name="gameplayak", on_delete=models.PROTECT
    )
    ...
    bideo_plataforma = models.ForeignKey(
        BideoPlataforma,
    ...
    def get_puntuak(self):
        if self.puntuak == 0:
            return 0
        return round(self.puntuak / self.botoak, 0.5)
    ...
    def get_bideo_url(self):
        if self.bideo_plataforma:
            ...
            return "https://www.youtube-nocookie.com/embed/%s" % (self.bideoa)
    ...
    def getTwitText(self):
        if self.erabiltzailea.twitter_id:
            return (
                truncatechars(self.izenburua, 60)
                + " @%s 2dz " % (self.erabiltzailea.twitter_id)
    ...
    def getTootText(self):
        if self.erabiltzailea.mastodon_id:
            return (
                truncatechars(self.izenburua, 230)
                + " @%s 2dz " % (self.erabiltzailea.mastodon_id)
                + self.get_absolute_url()
            )
        else:
    ...
    def getTelegramText(self):
        return (
            self.izenburua
            + " "
            + self.erabiltzailea.getFullName()
            + " 2dz "
            + self.get_absolute_url()
        )
