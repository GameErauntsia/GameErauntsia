...
def txapelketa(request,slug):
    ...
    if item.mota == '0' or item.mota == '2':
        if Partida.objects.filter(Q(txapelketa=item),Q(parent__isnull=True),Q(Q(txapelketa__mota='0')|(Q(txapelketa__mota='2'),Q(is_playoff=True))).exists():
            if item.mota == '0':
                first_node = Partida.objects.get(txapelketa=item,parent__isnull=True)
            else:
                first_node = Partida.objects.get(txapelketa=item,parent__isnull=True,is_playoff=True)
