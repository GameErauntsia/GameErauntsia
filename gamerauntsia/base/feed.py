...
class LatestEntriesFeed(Feed):
    ...
    def items(self):
        return GamePlaya.objects.filter(status='1',publikoa_da=True, pub_date__lt=timezone.now()).order_by('-pub_date')[:20]
...
class LatestNewsFeed(Feed):
    ...
    def items(self):
        return Berria.objects.filter(status='1', pub_date__lt=timezone.now()).order_by('-pub_date')[:20]
