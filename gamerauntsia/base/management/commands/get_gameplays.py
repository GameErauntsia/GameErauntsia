...
def get_gameplays():
    ...
    for gamer in gamers:
        ...
        channel = (
            "http://gdata.youtube.com/feeds/api/users/" + user + "/uploads?v=2&alt=json"
        )
        print(channel)
        data = get_urljson(channel)

        for video in data["feed"]["entry"][:5]:
            if not GamePlaya.objects.filter(
                bideoa=video["media$group"]["yt$videoid"]["$t"], erabiltzailea=gamer
            ).exists():
                if not AutoGamePlaya.objects.filter(
                    bideoa=video["media$group"]["yt$videoid"]["$t"], erabiltzailea=gamer
                ).exists():
                    url = ""
                    ...
                    auto.iraupena_min = time.strftime(
                        "%M",
                        time.gmtime(
                            int(video["media$group"]["yt$duration"]["seconds"])
                        ),
                    )
                    auto.iraupena_seg = time.strftime(
                        "%S",
                        time.gmtime(
                            int(video["media$group"]["yt$duration"]["seconds"])
                        ),
