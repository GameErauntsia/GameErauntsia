...
def start_telebot():
    ...
    def command_hashtag(message):
        ...
        try:
            ...
            if (
                ...
                and GamerUser.objects.filter(telegram_id=message.from_user.id).exists()
            ):
                ...
                tb.send_message(
                    message.chat.id,
                    "Aupa %s! Mezua jasota \U0001F44D" % (message.from_user.first_name),
                )
            else:
                tb.send_message(
                    message.chat.id,
                    "Barkatu... nor zara? \U0001F605\nKonfiguratu zure Telegram /kodea eta saiatu berriz!",
                )

        except:
            tb.send_message(
                message.chat.id,
                "Barkatu %s, ez dut zure mezua ulertu \U0001F62D"
    ...
    def command_list(message):
        ...
        if "kaixo" in text:
            tb.send_message(
                message.chat.id, "Kaixo %s!" % (message.from_user.first_name)
            )
        elif "foroa" in text:
            ...
            if "-bilatu" in text:
                ...
                if not len(find) < 3:
                    topics = "\n".join(
                        "#GE" + str(t.id) + " - " + t.title
                        for t in Topic.objects.filter(title__icontains=find).order_by(
                            "title"
                        )
                    )
                ...
                categories = "\n".join(
                    "#" + c.title.replace(" ", "_")
                    for c in Category.objects.all().order_by("title")
                )
                ...
                forums = "\n".join(
                    "#" + f.title.replace(" ", "_")
                    for f in Forum.objects.filter(category__title=cat).order_by("title")
                )
                ...
                topics = "\n".join(
                    "#GE" + str(t.id) + " - " + t.title
                    for t in Topic.objects.filter(forums__title=foru).order_by("title")
                )
                msg += topics or "Ez da ezer aurkitu..."
            else:
            ...
            if message.chat.id < 0:
                tb.send_message(
                    message.chat.id,
                    "Mesedez %s, eskatu zure kodea niri zuzenean. Ez erabili taldeak horretarako"
                    % (message.from_user.first_name),
                )
            else:
                tb.send_message(
                    message.chat.id,
                    "Zure kodea: %d\n\nGorde zenbaki hau zure Game Erauntsiko profilean"
                    % (message.from_user.id),
                )
        else:
            cmd_lst = ""
